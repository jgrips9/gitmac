...
Variable `r names[9]` has the following frequency counts and summary statistics in association with Miles per Gallon. Check it ou.   
...
Above uses code chunk to show code used to create results. Also could do inline code just to show numbers. Inline code will make it appear a bit more organized. Numbers printed are *t statistic* and corresponding *p-value*. `r c(testres$statistic, testres$p.value)`
